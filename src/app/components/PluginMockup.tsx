...
export default function PluginMockup({ initialModule, className = '' }: { initialModule?: ModuleId; className?: string }) {
  ...
  const editFamily = (tokens: Variable[], base: Variable, next: string) => {
    const color = parseColor(next);
    if (!color) return;
    setEdits(prev => ({ ...prev, ...recolorFamily(tokens, base, color) }));
    setStatus('Color scale updated using this preset’s original steps and color format.');
  };
  ...
  const tokenField = (v: Variable, onChange = (next: string) => edit(v, next)) => <label className="demo-token" key={v.id}>
    <span title={v.name}>{v.name}</span>
    {v.type === 'COLOR' && <input aria-label={`${v.name} color picker`} type="color" value={/^#[\da-f]{6}$/i.test(value(v)) ? value(v) : hex(parseColor(value(v)) ?? parseColor(v.displayValue)!, false)} onChange={e => {
